                  ...
                  <div className="flex items-center gap-1">
                      <button 
                          onClick={async () => {
                              if (language === 'English') {
                                  // Switching to Hindi
                                  if (!content.schoolPremiumNotesHtml_HI && !content.competitionPremiumNotesHtml_HI && !content.manualMcqData_HI) {
                                      // AUTO GENERATE HINDI
                                      const confirmGen = window.confirm("Hindi translation not found. Generate via AI (1500 words)? This will be saved forever.");
                                      if (confirmGen) {
                                          setLanguage('Hindi'); // Switch UI to show loading/streaming
                                          // Note: The parent component or a service needs to handle this generation and update 'content'.
                                          // Since we don't have the generate function passed here, we might need to handle it or show a message.
                                          // For this specific requirement ("Generate... and save forever"), logic is best placed in `fetchLessonContent` or a handler passed to LessonView.
                                          // However, LessonView receives the `content` object. Updating it requires `onUpdateContent` prop or similar.
                                          // Given the constraints, I will trigger an alert for now, or if `onUpdateUser` can be repurposed? No.
                                          // I will implement the generation logic here if I can import `translateToHindi` or `fetchLessonContent`.
                                          // I will import `translateToHindi` from services/groq.
                                          
                                          try {
                                               const { translateToHindi } = await import('../services/groq');
                                               const { saveChapterData } = await import('../firebase');
                                               
                                               // Determine what to translate
                                               const sourceText = content.schoolPremiumNotesHtml || content.content;
                                               if (sourceText) {
                                                   // Show loading
                                                   const loadingMsg = "Translating... Please wait.";
                                                   // We can manipulate DOM or local state to show loading.
                                                   alert(loadingMsg);
                                                   
                                                   const hindiText = await translateToHindi(sourceText, false, 'STUDENT');
                                                   
                                                   // Update Content Object
                                                   const updatedContent = { ...content, schoolPremiumNotesHtml_HI: hindiText, competitionPremiumNotesHtml_HI: hindiText };
                                                   
                                                   // Save to Storage/Firebase
                                                   // We need to reconstruct the key. Ideally passed as prop, but we can reconstruct.
                                                   ...
                                                   // Force reload/update? We can mutate the prop object locally since it's a ref in JS but React won't re-render.
                                                   // Better to rely on parent re-render or reload.
