...
import { HeroLayer, SmartSlideLayer } from './student/DashboardLayers'; // NEW IMPORTS
...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating, onNavigateToChapter, isDarkMode, onToggleDarkMode }) => {
  ...
  const [isVoiceEnabled, setIsVoiceEnabled] = useState(false); // Default OFF or load from settings

  const playGuide = (text: string) => {
  ...
  const renderMainContent = () => {
      // 1. HOME TAB (3-LAYER MASTER PLAN)
      if (activeTab === 'HOME') { 
          // New 3-Layer Layout
          return (
              <>
                  <HeroLayer 
                      ...
                      onSetTarget={setDailyTargetSeconds}
                      weeklyData={[]} // TODO: Fetch weekly data
                      streak={user.streak}
                  ...
                  <SmartSlideLayer>
                      ...
                      <div className="flex justify-between items-center px-2">
                          <h3 className="font-black text-slate-800 text-lg">Explore</h3>
                          <button 
                              onClick={() => {
                                  if (isVoiceEnabled) stop();
                                  else speak("Audio Guide Enabled. Tap any button to hear details.");
                                  setIsVoiceEnabled(!isVoiceEnabled);
                              }}
                              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all ${isVoiceEnabled ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-500'}`}
                          >
                              {isVoiceEnabled ? <Volume2 size={14} /> : <Headphones size={14} />}
                      ...
                      {/* NOTIFICATION BAR */}
                      {settings?.noticeText && (
                          <div className="bg-slate-900 text-white p-3 rounded-xl shadow-md border border-slate-700 animate-in slide-in-from-top-4 relative mx-1">
                      ...
                      <BannerCarousel>
                          {/* AI TOUR (GLOWING) */}
                          <div 
                              onClick={() => { playGuide("Welcome to AI Tour. Here you can learn how to use the app."); setShowUserGuide(true); }}
                              className="mx-1 h-48 bg-gradient-to-r from-purple-600 to-indigo-600 p-6 rounded-3xl shadow-xl shadow-purple-200 relative overflow-hidden cursor-pointer group border border-white/20"
                          >
                              <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-30 animate-pulse"></div>
                              <div className="relative z-10 flex flex-col justify-center h-full">
                                  <h2 className="text-3xl font-black text-white mb-2">AI TOUR</h2>
                                  <p className="text-purple-200 text-sm font-medium">Master the app in 2 minutes.</p>
                                  <div className="mt-4 flex items-center gap-2 text-white font-bold text-xs bg-white/20 w-fit px-3 py-1.5 rounded-lg backdrop-blur-md">
                          ...
                          {slides.map((slide, index) => (
                              <div 
                                  key={slide.id}
                                  className={`absolute inset-0 flex flex-col justify-center p-6 transition-all duration-1000 ${index === currentSlide ? 'opacity-100 scale-100' : 'opacity-0 scale-95'} mx-1 h-48 rounded-2xl overflow-hidden shadow-xl border border-white/20`}
                      ...
                      {/* SERVICES GRID (Audio Guide Integration) */}
                      <DashboardSectionWrapper id="services_grid" label="Services Grid">
                          <div className="grid grid-cols-3 gap-3">
                              {[
                                  { id: 'inbox', label: 'Inbox', icon: Mail, onClick: () => setShowInbox(true), desc: "Check your messages and rewards here." },
                                  { id: 'analytics', label: 'Analytics', icon: BarChart3, onClick: () => onTabChange('ANALYTICS'), desc: "View your study progress and reports." },
                                  { id: 'marksheet', label: 'Marksheet', icon: FileText, onClick: () => setShowMonthlyReport(true), desc: "See your monthly performance card." },
                                  { id: 'history', label: 'History', icon: History, onClick: () => onTabChange('HISTORY'), desc: "Review your past activities." },
                                  { id: 'ai_history', label: 'AI History', icon: BrainCircuit, onClick: () => onTabChange('AI_HISTORY'), desc: "Check your AI interactions." },
                                  { id: 'store', label: 'Store', icon: Crown, onClick: () => onTabChange('STORE'), desc: "Buy credits and premium plans." },
                                  { id: 'game', label: 'Game', icon: Gamepad2, onClick: () => onTabChange('GAME'), desc: "Play spin wheel to win credits." },
                                  { id: 'redeem', label: 'Redeem', icon: Gift, onClick: () => onTabChange('REDEEM'), desc: "Redeem your coupon codes." },
                                  { id: 'ranks', label: 'Ranks', icon: Trophy, onClick: () => onTabChange('LEADERBOARD'), desc: "Check the leaderboard." },
                              ].map(item => (
                                  <DashboardTileWrapper key={item.id} id={`tile_${item.id}`} label={item.label}>
                                      <button 
                                          onClick={() => { playGuide(item.desc); item.onClick(); }}
                                          className="h-20 w-full bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col items-center justify-center gap-1 hover:scale-[1.02] transition-transform relative overflow-hidden"
                                      >
                                          <div className={`p-2 rounded-full ${activeTab === item.id.toUpperCase() ? 'bg-blue-100 text-blue-600' : 'bg-slate-50 text-slate-500'}`}>
                                              <item.icon size={20} />
                                          </div>
                                          <span className="text-[10px] font-black text-slate-700 uppercase tracking-wider">{item.label}</span>
                                      </button>
