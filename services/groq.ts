...
export const fetchLessonContent = async (
  ...
  const generateNotes = async (detailed: boolean): Promise<{text: string, hindiText?: string}> => {
      ...
      if (template) {
          ...
          if (detailed) {
              // PREMIUM PROMPT (1000-1500 Words) - GEMINI STYLE ENFORCED
              prompt = `${customInstruction}
              ${adminPromptOverride || ""}
              ...
              STRICT TARGET: 1000-1500 Words. (Do not write less than 1000 words).
              
              STYLE: "Gemini Style" - Conversational, Deep Analysis, Student-Friendly.
              - Speak directly to the student ("Hey Student, let's understand this...").
              - Use Analogies and Real-world examples.
              - Explain "Why" before "What".
              
              STRUCTURE:
              1. 🌟 Introduction (Hook with a real-life example or Thinking Question)
              2. 📘 Detailed Concept Breakdown (Step-by-step, deep analysis of every topic)
              3. 📊 Text-Based Diagrams / Flowcharts (Use ASCII arrows e.g. Sun -> Plant -> Herbivore)
              4. 🧠 "Concept Chamka?" Deep Dive (The logic/science behind the concept)
              5. 🧪 Real World Examples & Case Studies
              6. ⚠️ Exam Alerts (Common Mistakes & Exam Traps)
              7. 🏆 Topper's Trick (Mnemonics & Shortcuts)
              8. 📝 20 Practice MCQs (At the very end, with Answer Key and Detailed Solutions)
              
              Use bold text for keywords. Make it comprehensive and engaging.`;
          } else {
